...
public class DATAVIEW_MathMatrix {
	...
	public void addNewRow(DATAVIEW_MathVector newvt) {
		if (n != newvt.length()) {
			throw new IllegalArgumentException("Dimensions are not equal for the plus operation.");
		}
		for (int i = 0; i < n; i++)
	...
	public void addRow(int row, DATAVIEW_MathVector newv) {
		Dataview.debugger.logFalseCondition(
				"The new vector should have the same dimenstion of a a row in the matrix, newv.length == row.length",
				n == newv.length());
		for (int j = 0; j < n; j++)
	...
	/*
	 * Given an input vector, which row vector is closest to the input vector? This
	 * method will return the index of such a row.
	 */
